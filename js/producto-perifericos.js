...
<div class="card bg-light mb-3" style="max-width: 20rem;">
  ...
  <div class="card-body">
   ...
   <li>CPU: ${element.marca}</li>
   <li>CPU: RAM:  ${element.potencia}</li>
   <li>Tarjeta gráfica: ${element.tipo_de_uso}</li>
   <li>CPU: ${element.fuente_de_alimentacion}</li>
   <li>Conexiones traseras: ${element.conexiones}</li>
</ul>
    </p>
    <p class="card-text">
    ${element.descripcion}
...
for (let i = 0; i < perifericos.length; i++) {
   ...
   if (perifericos[i].id === parseInt(userId)) {
      const element = perifericos[i];
...
document.addEventListener("click", (ev) => {
  if (ev.target.classList.contains("ver-carrito")) {
    ...
    for (let i = 0; i < perifericos.length; i++) {
      ...
      if (perifericos[i].id === parseInt(userId)) {
        const elementocarrito = perifericos[i];
